...
# Ajuster les poids de classe pour réduire le biais
weights = compute_class_weight(class_weight='balanced', classes=np.unique(labels_train), y=labels_train)
class_weights = dict(zip(np.unique(labels_train), weights))
...
# Réévaluer le modèle avec les nouveaux poids de classe
X_train, X_val, y_train, y_val = train_test_split(features_train_scaled, labels_train, test_size=0.2, random_state=42)
best_rf.fit(X_train, y_train)
y_pred_val = best_rf.predict(X_val)
...
# Prédire les catégories des nouvelles images avec le modèle ajusté
predictions = best_rf.predict(features_test_scaled)
